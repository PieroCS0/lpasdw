...
(function () {
  ...
  /* ---------------------------------------------------------
     Construcción de la flor central (estado 2) y del girasol
     inicial (estado 1)
     --------------------------------------------------------- */
  ...
  startFlowerBtn.innerHTML = makeCentralFlowerSVG();
  ...
  /* ---------------------------------------------------------
     ...
     Orden pedido: primero aparece el universo (estrellas, ya
     corriendo), luego las flores del campo, y al final el
     mensaje/título — no al revés.
     --------------------------------------------------------- */
  function playUniverseIntro() {
    ...
    var t = 250; // el cielo ya está dibujándose; damos un instante antes del anillo
    ...
    for (var i = 0; i < initialCount; i++) {
      addFieldFlower(t + i * 130);
    }
    t += initialCount * 130 + 350;
  ...
  function onStartTap() {
    ...
    // ✨ chispas al presionar
    spawnBurst(cx, cy, 30, GOLD_TONES);

    // destello mágico de transformación
    flashOverlay.classList.remove('play');
    void flashOverlay.offsetWidth;
    flashOverlay.classList.add('play');

    // el estado 1 se desvanece
    stage1.classList.add('fade-out');

    setTimeout(function () {
      stage1.style.display = 'none';
      appEl.classList.remove('hidden');
      // reflow antes de animar la opacidad de entrada
  ...
  /* ---------------------------------------------------------
     ...
     estado 1). El universo (estrellas, partículas, flores) no
     se dibuja ni se construye hasta que el usuario presiona el
     girasol inicial — así la pantalla de apertura es minimalista
     de verdad y no cuesta nada de rendimiento.
     --------------------------------------------------------- */
  sizeBurstCanvas();
